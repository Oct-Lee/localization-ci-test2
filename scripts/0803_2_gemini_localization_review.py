...
# High-confidence VALUE typos that Gemini may miss in large batches.
# (wrong_substring, correct_substring, problem)
...
- Chinese character typos are HIGH. Example: "计时器己启动" → "计时器已启动"
  (己/已). Check EVERY user_facing VALUE; do not skip early entries in
  long files.
...
def call_gemini(api_key: str, prompt: str) -> tuple[dict[str, Any], float]:
    ...
    body = {
        ...
        # Low temperature: more stable spelling / typo detection on large diffs.
...
def deterministic_known_typo_issues(
    ...
    """Emit HIGH issues for known VALUE typos (Gemini large-batch safety net)."""
    found: list[dict[str, Any]] = []
    seen: set[tuple[str, int, str]] = set()
    for path, rows in added_details.items():
        for row in rows:
            values = extract_user_facing_hints(row.get("text", ""), path)
            for value in values:
                    ...
                    found.append(
                        {
                            ...
                            "original": value,
                            "problem": problem,
                            "suggestion": value.replace(wrong, right),
...
def merge_issues(
    ...
    """Append extra issues not already present (by file + original + suggestion)."""
    keys = {
        (i.get("file"), i.get("original"), i.get("suggestion")) for i in primary
    }
    merged = list(primary)
    for issue in extra:
...
def postprocess_issues(
    ...
    """Attach locations, drop FPs, merge known typo safety-net, force identifier LOW."""
    ...
    if known:
        ...
        if added:
            print(
                f"Added {added} known typo issue(s) via deterministic safety net",
                file=sys.stderr,
            )
